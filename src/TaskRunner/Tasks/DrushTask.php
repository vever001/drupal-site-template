 ...
 *
 * This class is mostly a verbatim copy from Acquia BLT.
 *
 * @see https://github.com/acquia/blt/blob/12.x/src/Robo/Tasks/DrushTask.php
 *
...
class DrushTask extends CommandStack {
  ...
  protected function init() {
    if ($this->getConfig()->get('runner.bin_dir')) {
      $this->executable = $this->getConfig()->get('runner.bin_dir') . '/drush';
    }
    else {
    ...
    if (!isset($this->dir)) {
      $this->dir($this->getConfig()->get('drush.options.dir'));
    }
    if (!isset($this->uri)) {
      $this->uri = $this->getConfig()->get('drush.options.uri');
    }
    if (!isset($this->alias)) {
      $this->alias($this->getConfig()->get('drush.options.alias'));
    }
    if (!isset($this->interactive)) {
      $this->interactive(FALSE);
    }
    if (!isset($this->ansi)) {
      $this->ansi($this->getConfig()->get('drush.options.ansi'));
    }
  ...
  protected function setGlobalOptions() {
    ...
    if (($this->debug || $this->getConfig()->get('drush.options.debug'))
      && $this->getConfig()->get('drush.options.debug') !== FALSE) {
      $this->option('-vvv');
    }
    elseif (($this->veryVerbose || $this->getConfig()->get('drush.options.veryVerbose'))
      && $this->getConfig()->get('drush.options.veryVerbose') !== FALSE) {
      $this->option('-vv');
    }
    elseif (($this->verbose || $this->getConfig()->get('drush.options.verbose'))
      && $this->getConfig()->get('drush.options.verbose') !== FALSE) {
      $this->option('-v');
    }
